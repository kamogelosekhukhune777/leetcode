...
the circuit once in the clockwise direction, otherwise return -1. If there exists a solution, it
...
// canCompleteCircuit finds the starting gas station index if you can complete the circuit
...
func main() {
	gas1 := []int{1, 2, 3, 4, 5}
	cost1 := []int{3, 4, 5, 1, 2}
	fmt.Println(canCompleteCircuit(gas1, cost1)) // Output: 3

	gas2 := []int{2, 3, 4}
	cost2 := []int{3, 4, 3}
	fmt.Println(canCompleteCircuit(gas2, cost2)) // Output: -1
}
