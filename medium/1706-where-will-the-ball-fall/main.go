...
/*
You have a 2-D grid of size m x n representing a box, and you have n balls. The box is open on the top
and bottom sides.
...
func main() {
	...
	fmt.Println(findBall(grid1)) // Output: [1, -1, -1, -1, -1]

	grid2 := [][]int{{-1}}
	fmt.Println(findBall(grid2)) // Output: [-1]
	...
	fmt.Println(findBall(grid3)) // Output: [0, 1, 2, 3, 4, -1]
}
